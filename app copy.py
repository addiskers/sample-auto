...
FALLBACK_RESPONSES = {
    ...
    AIRequestType.COMPANY_REVENUE: [
      ...
      AIRequestType.RESEARCH_JOURNALS: [
    ...
    AIRequestType.INDUSTRY_ASSOCIATIONS: [
...
class AIService:
    ...
    def _generate_research_journals(self, context: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if len(journals) < 5:
                fallback_journals = self._get_fallback_response(AIRequestType.RESEARCH_JOURNALS, context)
                journals.extend(fallback_journals[len(journals):5])
            
            return journals[:5]  # Return exactly 5 journals
            
        except Exception as e:
            print(f"Failed to generate research journals: {str(e)}")
            return self._get_fallback_response(AIRequestType.RESEARCH_JOURNALS, context)
    ...
    def _generate_industry_associations(self, context: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if len(associations) < 5:
                fallback_associations = self._get_fallback_response(AIRequestType.INDUSTRY_ASSOCIATIONS, context)
                associations.extend(fallback_associations[len(associations):5])
            
            return associations[:5]  # Return exactly 5 associations
            
        except Exception as e:
            print(f"Failed to generate industry associations: {str(e)}")
            return self._get_fallback_response(AIRequestType.INDUSTRY_ASSOCIATIONS, context)
...
def generate_ppt():
    try:
        ...
        required_fields = [
            ...
            'rev_current', 'rev_future',  'segment_input'
        ]
